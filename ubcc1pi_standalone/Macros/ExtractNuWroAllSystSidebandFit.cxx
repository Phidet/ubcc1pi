/**
 *  @file  ubcc1pi_standalone/Macros/ExtractNuWroAllSystSidebandFit.cxx
 *
 *  @brief The implementation file of the ExtractNuWroAllSystSidebandFit macro
 */
...
const ubsmear::UBMatrix *pPredictionNuWroAllSyst, *pDataNuWroAllSyst, *pSmearingMatrixNuWroAllSyst, *pPredictionErrorMatrixNuWroAllSyst, *pDataEigenAllSyst, *pTransformationSmearingMatrixAllSyst, *pEigenvaluesAllSyst; //, *pTotalErrorMatrixMapAllSystDataNuWroAllSyst, *pTotalErrorMatrixMapAllSystSmearingMatrixNuWroAllSyst;//, *pPredictionErrorMatrixNuWroAllSyst;
const std::map<std::string, ubsmear::UBMatrix> *pTotalErrorMatrixMapAllSyst;
const ubsmear::UBXSecMeta *pMetadataNuWroAllSyst;

const auto testingFractionSystAll = 1.f;
const auto rateOnlySystAll = true;

// void GetChiSquaredAllSyst(Int_t &npar, Double_t *gin, Double_t &chi2, Double_t *par, Int_t iflag)
// {
...
//     const auto nBins = pMetadataNuWroAllSyst->GetNBins();
//     for (unsigned int i = 0; i<nBins; ++i) paramVec.push_back(par[i]);
//     const ubsmear::UBMatrix parameters(paramVec, paramVec.size(), 1);
//     const auto parameterScaledPrediction = ElementWiseOperation(*pPredictionNuWroAllSyst, parameters, [](const auto &l, const auto& r) { return l * r; });

//     const auto smearedPrediction = ubsmear::UBSmearingHelper::Smear(*pMetadataNuWroAllSyst, parameterScaledPrediction, *pSmearingMatrixNuWroAllSyst);
//     const auto diff = smearedPrediction - *pDataNuWroAllSyst;
//     const auto trimmedDiff = trim ? ubsmear::UBSmearingHelper::TrimUnderOverflowBins(diff, *pMetadataNuWroAllSyst) : diff;
//     const auto trimmedDataErrorNuWro = trim ? ubsmear::UBSmearingHelper::TrimUnderOverflowBins(pTotalErrorMatrixMapAllSyst->at("data"), *pMetadataNuWroAllSyst): pTotalErrorMatrixMapAllSyst->at("data");
//     const auto trimmedParameters = trim ? ubsmear::UBSmearingHelper::TrimUnderOverflowBins(parameters, *pMetadataNuWroAllSyst) : parameters;

//     // chi2 = 0;
//     // for (unsigned int i=0; i<trimmedDiff.GetRows(); i++)
//     // {
//     //     if(pDataNuWroAllSyst->At(i, 0)<std::numeric_limits<float>::epsilon() && pPredictionNuWroAllSyst->At(i, 0)<std::numeric_limits<float>::epsilon()) continue; //todo improve so that this check is not needed every time
...
//     const auto &[chiSquared, degreesOfFreedom] = ubsmear::UBStatisticsHelper::GetChi2(smearedPrediction, *pDataNuWroAllSyst, trimmedDataErrorNuWro, precision);
//     chi2 = chiSquared;
// }

std::pair<ubsmear::UBMatrix, ubsmear::UBMatrix> GetTransformationMatrixAllSyst()
{
    const auto precision = 1e-5;
    const auto covarianceMatrix = pTotalErrorMatrixMapAllSyst->at("data");
...
void GetChiSquaredAllSyst(Int_t &npar, Double_t *gin, Double_t &chi2, Double_t *par, Int_t iflag)
{
    // std::cout<<"GetChiSquaredAllSyst - Debug 0"<<std::endl;
    std::vector<float> paramVec;
    // std::cout<<"GetChiSquaredAllSyst - Debug 0.1"<<std::endl;
    const auto nBins = pMetadataNuWroAllSyst->GetNBins();
    // std::cout<<"GetChiSquaredAllSyst - Debug 0.2"<<std::endl;
    for (size_t iBin = 0; iBin < nBins; ++iBin) paramVec.push_back(par[iBin]);
    // std::cout<<"GetChiSquaredAllSyst - Debug 0.3"<<std::endl;
    const ubsmear::UBMatrix parameters(paramVec, paramVec.size(), 1);
    // std::cout<<"GetChiSquaredAllSyst - Debug 0.4"<<std::endl;
    const auto parameterScaledPrediction = ElementWiseOperation(*pPredictionNuWroAllSyst, parameters, [](const auto &l, const auto& r) { return l * r; });
    // std::cout<<"GetChiSquaredAllSyst - Debug 0.5"<<std::endl;

    const auto smearedPredictionEigen = ubsmear::UBSmearingHelper::Smear(*pMetadataNuWroAllSyst, parameterScaledPrediction, *pTransformationSmearingMatrixAllSyst);
    // std::cout<<"GetChiSquaredAllSyst - Debug 1"<<std::endl;

    // Get the difference between the smeared prediction and the data in the eigenbasis
    const auto diffEigen = *pDataEigenAllSyst - smearedPredictionEigen;
    ...
    for (size_t iBin = 0; iBin < nBins; ++iBin)
    {
        ...
        const auto variance = pEigenvaluesAllSyst->At(iBin, 0);
...
void ExtractNuWroAllSystSidebandFit(const Config &config)
{
    // -------------------------------------------------------------------------------------------------------------------------------------
    ...
    auto systParamsNuWroTruth = systParams; // shallow copy sufficient since struct contains no pointers
    systParamsNuWroTruth.potFracUncertainty = 0.0;
    systParamsNuWroTruth.fluxDimensions = emptySystDimMap;
    systParamsNuWroTruth.reintDimensions = emptySystDimMap;
    ...
    })
    {
        ...
        for (const auto &selectionName : {"generic", "golden"})
        {
            ...
            if (config.extractXSecs.crossSectionIsEnabled.at(selectionName).at(name))
            {
                ...
                xsecMapSidebandBNB[selectionName].emplace(name, CrossSectionHelper::CrossSection(systParams, extendedBinEdgesReco, hasUnderflowReco, hasOverflowReco, scaleByBinWidth, extendedBinEdgesTruth, rateOnlySystAll));
                xsecMapSidebandNuWro[selectionName].emplace(name, CrossSectionHelper::CrossSection(systParamsNuWro, extendedBinEdgesReco, hasUnderflowReco, hasOverflowReco, scaleByBinWidth, extendedBinEdgesTruth, rateOnlySystAll));
                xsecMapSidebandGenie[selectionName].emplace(name, CrossSectionHelper::CrossSection(systParamsGenie, extendedBinEdgesReco, hasUnderflowReco, hasOverflowReco, scaleByBinWidth, extendedBinEdgesTruth, rateOnlySystAll));
                xsecMapSidebandNuWroTrue[selectionName].emplace(name, CrossSectionHelper::CrossSection(systParamsNuWroTruth, extendedBinEdgesReco, hasUnderflowReco, hasOverflowReco, scaleByBinWidth, extendedBinEdgesTruth, rateOnlySystAll));
    ...
    for (const auto &selectionName : {"generic", "golden"})
    {
        ...
        {
            xsecMapSidebandBNB[selectionName].emplace("total", CrossSectionHelper::CrossSection(systParams, {-1.f, 1.f}, false, false, false, std::vector<float>(), rateOnlySystAll));
            xsecMapSidebandNuWro[selectionName].emplace("total", CrossSectionHelper::CrossSection(systParamsNuWro, {-1.f, 1.f}, false, false, false, std::vector<float>(), rateOnlySystAll));
            xsecMapSidebandGenie[selectionName].emplace("total", CrossSectionHelper::CrossSection(systParamsGenie, {-1.f, 1.f}, false, false, false, std::vector<float>(), rateOnlySystAll));
            xsecMapSidebandNuWroTrue[selectionName].emplace("total", CrossSectionHelper::CrossSection(systParamsNuWroTruth, {-1.f, 1.f}, false, false, false, std::vector<float>(), rateOnlySystAll));
        }
    }
    ...
    scalingData.exposurePOT = totalExposurePOT*testingFractionSystAll;
    ...
    for (const auto &[sampleType, sampleName, fileName, normalisation] : inputData)
    {
        ...
        //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        std::cout<<"############################\nUsing "<<testingFractionSystAll*100<<"\% of events!\n############################\n"<<std::endl;
        for (unsigned int i = 0; i < nEvents*testingFractionSystAll; ++i) // todo change back to all events!!!!!!!!!!!!!!!!!!!!!!
        {
            // std::cout<<"DEBUG Fit P0"<<std::endl;
            AnalysisHelper::PrintLoadingBar(i, int(nEvents*testingFractionSystAll));
            reader.LoadEvent(i);
                ...
                for (auto &[selectionName, xsecs] : xsecMapSidebandBNB)
                {
                    ...
                    if (isSignal)
                    {
                        ...
                        for (auto &[name, xsec] : xsecs)
                        {
                            ...
                            xsecMapSidebandNuWro.at(selectionName).at(name).AddSignalEventDetVar(recoValue, trueValue, isSelected, weight, sampleName);
                        }
                    }
                    ...
                    else
                    {
                        for (auto &[selectionName, xsecs] : xsecMapSidebandBNB)
                        {
                            ...
                            for (auto &[name, xsec] : xsecs)
                            {
                                ...
                                xsecMapSidebandNuWro.at(selectionName).at(name).AddSelectedBackgroundEventDetVar(recoValue, weight, sampleName);
                            }
                        }
                    }
                }
            ...
            for (auto &[selectionName, xsecs] : xsecMapSidebandNuWro)
            {
                ...
                if (isSignal)
                {
                    ...
                    for (auto &[name, xsec] : xsecs)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // std::cout<<"DEBUG Fit P8.5 - fluxWeights.size(): "<<fluxWeights.size()<<std::endl;
                                if(config.global.useNuWroAsData) xsec.AddSignalEvent(recoValue, trueValue, isSelected, weight, fluxWeights, xsecWeights, reintWeights);
                                // std::cout<<"DEBUG Fit P8.5.1"<<std::endl;
                ...
                else if (isSelected)
                {
                    ...
                    for (auto &[name, xsec] : xsecs)
                    {
                        ...
                        if(isNuWro)
                        {
                            // std::cout<<"DEBUG Fit P10.1"<<std::endl;
                        ...
                        else
                        {
                            if(isOverlay) // Genie MC for NuWro (no EXT or dirt events in NuWro)
                            {
                                // std::cout<<"DEBUG Fit P10.2"<<std::endl;
                                if(config.global.useNuWroAsData) xsec.AddSelectedBackgroundEvent(recoValue, false, weight, fluxWeights, xsecWeights, reintWeights, bootstrapWeights);
    ...
    for (const auto &[dataTypeName, xsecMap] : xsecMapVector) // {std::make_pair(std::string("NuWro"), xsecMapSidebandNuWro), std::make_pair(std::string("BNB"), xsecMapSidebandBNB)})
    {
        for (const auto &[selectionName, xsecs] : xsecMap)
        {
            for (const auto &[name, xsec] : xsecs)
            {
                ...
                std::cout<<"_______________________ExtractNuWroAllSystSidebandFit Fitting: "<<dataTypeName<<" - "<<selectionName<<" - "<<name<<"_______________________"<<std::endl;
                ...
                // pPredictionErrorMatrixNuWroAllSyst = &predictionErrorMatrix;
                pMetadataNuWroAllSyst = &metadata;
                pSmearingMatrixNuWroAllSyst = &smearingMatrixGeneral; //todo check which smearing matrix is needed
                pPredictionNuWroAllSyst = &prediction;
                pDataNuWroAllSyst = &data;
                pTotalErrorMatrixMapAllSyst = &totalErrorMatrixMap; // todo check whether individual universe uncertainties need to be generated (due to differenced is genie backgrounds in each universe)
                ...
                const auto [eigenvalues, transformationMatrix] = GetTransformationMatrixAllSyst();
                // Transform the smeared prediction and input data into the basis of eigenvectors
                const auto dataEigen = transformationMatrix*(*pDataNuWroAllSyst);
                const auto transformationSmearingMatrix = transformationMatrix*(*pSmearingMatrixNuWroAllSyst);
                // std::cout<<"GetTransformationMatrixAllSyst - Debug 0"<<pSmearingMatrixNuWroAllSyst->GetRows()<<" "<<pSmearingMatrixNuWroAllSyst->GetColumns()<<transformationMatrix.GetRows()<<" "<<transformationMatrix.GetColumns()<<std::endl;
                pTransformationSmearingMatrixAllSyst = &transformationSmearingMatrix;
                // std::cout<<"GetTransformationMatrixAllSyst - Debug 1"<<std::endl;
                pDataEigenAllSyst = &dataEigen;
                // std::cout<<"GetTransformationMatrixAllSyst - Debug 2"<<std::endl;
                pEigenvaluesAllSyst = &eigenvalues;
                // std::cout<<"GetTransformationMatrixAllSyst - Debug 3"<<std::endl;
                // std::cout<<"DEBUG Nom Fit Point -0.5"<<std::endl;

                minimizer.Fit(GetChiSquaredAllSyst, result, successful, fitCovMatrixVector, 1);
                ...
                if(!successful)
                {
                    std::cout<<"ERROR: ExtractNuWroAllSystSidebandFit - Fit failed."<<std::endl;
                    continue; //todo remove!!!!!!!!!!!!!!!!!!!!
                    throw std::logic_error("ERROR: ExtractNuWroAllSystSidebandFit - Nominal fit failed.");
                }
                // std::cout<<"DEBUG Nom Fit Point 1"<<std::endl;
                ...
                if(dataTypeName=="BNB" || dataTypeName=="NuWro")
                {
                ...
                for (const auto &[group, dimensions] : weightDimensionMap)
                {
                    for (const auto &[paramName, nUniverses] : dimensions)
                    {
                        ...
                        for (unsigned int iUni = 0; iUni < nUniverses; ++iUni)
                        {
                            ...
                            if(pSmearingMatrixNuWroAllSyst)
                            {
                                ...
                                pDataNuWroAllSyst = &dataInUniverse;
                                pSmearingMatrixNuWroAllSyst = &smearingMatrixInUniverse;
                                pPredictionNuWroAllSyst = &predictionInUniverse;
                                //todo: check if totalerrormatrixmap also needs to be calaculated in each universe

                                // std::cout<<"+ExtractNuWroAllSystSidebandFit Fitting Point 2"<<std::endl;
                                std::vector<float> covMatrixInUniverse;
                                bool successful = false;

                                // std::cout<<"+ExtractNuWroAllSystSidebandFit Fitting Point 3"<<std::endl;
                                const auto [eigenvaluesInUniverse, transformationMatrixInUniverse] = GetTransformationMatrixAllSyst();
                                const auto dataEigenInUniverse = transformationMatrixInUniverse*(*pDataNuWroAllSyst);
                                const auto transformationSmearingMatrixInUniverse = transformationMatrixInUniverse*(*pSmearingMatrixNuWroAllSyst);
                                // std::cout<<"GetTransformationMatrixAllSystInUniverse - Debug 0"<<pSmearingMatrixNuWroAllSyst->GetRows()<<" "<<pSmearingMatrixNuWroAllSyst->GetColumns()<<transformationMatrixInUniverse.GetRows()<<" "<<transformationMatrixInUniverse.GetColumns()<<std::endl;
                                pTransformationSmearingMatrixAllSyst = &transformationSmearingMatrixInUniverse;
                                // std::cout<<"GetTransformationMatrixAllSystInUniverse - Debug 1"<<std::endl;
                                pDataEigenAllSyst = &dataEigenInUniverse;
                                // std::cout<<"GetTransformationMatrixAllSystInUniverse - Debug 2"<<std::endl;
                                pEigenvaluesAllSyst = &eigenvaluesInUniverse;
                                // std::cout<<"GetTransformationMatrixAllSystInUniverse - Debug 3"<<std::endl;
                                // minimizer.Fit(GetChiSquaredAllSyst, result, successful, fitCovMatrixVector, 1);

                                minimizer.Fit(GetChiSquaredAllSyst, resultInUniverse, successful, covMatrixInUniverse, -1, result.first); // uses nominal parameters as starting point
    ...
    std::ofstream ofs1("cc0piNominalConstraintMapAllSyst.bin", std::ios::binary);
    boost::archive::binary_oarchive oarch1(ofs1);
    oarch1 << cc0piNominalConstraintMap;
    ofs1.close();
    if(config.global.fitInSystematicUniverses)
    {
        std::ofstream ofs2("cc0piUniverseConstraintMapAllSyst.bin", std::ios::binary);
        boost::archive::binary_oarchive oarch2(ofs2);
