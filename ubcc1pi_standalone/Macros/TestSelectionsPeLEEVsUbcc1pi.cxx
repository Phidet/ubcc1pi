/**
 *  @file  ubcc1pi_standalone/Macros/TestSelectionsPeLEEVsUbcc1pi.cxx
 *
 *  @brief The implementation file of the TestSelectionsPeLEEVsUbcc1pi macro
 */
...
void TestSelectionsPeLEEVsUbcc1pi(const Config &config)
{
    ...
    for (unsigned int i = 0; i < nEvents; ++i)
    {
        ...
        std::cout<<"DEBUG point B0"<<std::endl;
        const auto isTrueCC1Pi = AnalysisHelper::IsTrueCC1Pi(pEvent, config.global.useAbsPdg);
        std::cout<<"DEBUG point B1"<<std::endl;
        const auto isTrueCC1PiPeLEE = AnalysisHelper::IsTrueCC1Pi(pEventPeLEEAsUbCC1Pi, config.global.useAbsPdg);
        std::cout<<"DEBUG point B2"<<std::endl;
        auto problematic = false;
        if(isTrueCC1Pi != isTrueCC1PiPeLEE)
        {
            std::cout<<"Problem: isTrueCC1Pi("<<isTrueCC1Pi<<") != isTrueCC1PiPeLEE("<<isTrueCC1PiPeLEE<<")"<<std::endl;
            problematic = true;
        }

        if(pEvent->truth.particles.size() != pEventPeLEEAsUbCC1Pi->truth.particles.size())
        {
            std::cout<<"Problem: pEvent->truth.particles.size() != pEventPeLEEAsUbCC1Pi->truth.particles.size()"<<std::endl;
            problematic = true;
        }
        if (problematic)
        {
            std::cout<<" +1 problematic"<<std::endl;
            nProblematic++;
        }
        ...
        // Check the selection
        // ******************************************************************************************
        const auto &[passedGoldenSelectionUbcc1pi, cutsPassedUbcc1pi, assignedPdgCodesUbcc1pi] = defaultSelection.Execute(pEvent);
        const auto passedGenericSelectionUbcc1pi = SelectionHelper::IsCutPassed(cutsPassedUbcc1pi, config.global.lastCutGeneric);
        const auto &[passedGoldenSelectionPelee, cutsPassedPelee, assignedPdgCodesPelee] = defaultSelection.Execute(pEventPeLEEAsUbCC1Pi);
        ...
        if(passedGenericSelectionPelee) nPassedGenericSelectionPeLEE++;
    ...
    std::cout<<"Passed generic selection PeLEE: "<<nPassedGenericSelectionPeLEE<<"/"<<nEvents<<std::endl;
    ...
    std::cout<<"Problematic: "<<nProblematic<<std::endl;
